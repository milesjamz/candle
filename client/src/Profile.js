...
// }
    ...
    return (
        <div>
            ...
            <ul>
            {props.user.following ? props.user.following.map(following => <UserCard update={props.update} currentUser={props.user} context={'following'} user={following} key={following.id} />) : null}
            </ul>
            ...
            Memories:<br/>
            Add memory:<select>
                { myAge > 0 ? [...Array(myAge).keys()].map(age => <option key={age}>{age}</option>) : null}
                </select><br/>
